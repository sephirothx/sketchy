...
| **R-ENG-15** | Coverage MUST be measured with **branch** coverage and gated **per risk-critical module**, not only in total. Branch, because a statement measure marks an `if` covered the moment it is reached, whatever the condition did — which is the half of "unexecuted branches" a statement count cannot see. Per module, because a suite this size absorbs one module losing its tests without moving the total more than a rounding error. A module named in the floor table that disappears from the report MUST fail, so a rename cannot retire a floor silently. [`scripts/check-coverage.py`](../scripts/check-coverage.py) |
| **R-ENG-16** | Every build MUST produce a CycloneDX SBOM for the shipped Python and frontend dependencies, generated from what resolves rather than from the manifest text, and MUST publish it **before** any policy is evaluated against it — the run worth having an inventory from is the one that just failed. That SBOM MUST be the input to the licence policy: one description of what ships, not two that can disagree. The policy MUST be an **allowlist**, failing closed on any identifier nobody has reviewed — a denylist only refuses the terms someone thought to name, and would pass `Elastic-2.0`. SPDX expressions MUST be **parsed against the grammar** and evaluated, not pattern-matched: `MIT OR GPL-3.0` passes on MIT, `(MIT OR GPL-3.0) AND SSPL-1.0` does not, and a malformed expression (`MIT OR`, `()`) MUST fail rather than evaluate to true. An SBOM listing no components MUST fail too — a generator that produced nothing is not a clean bill of health. [`scripts/check-licenses.py`](../scripts/check-licenses.py) |
...
| CI supply-chain and quality gates | [`.github/workflows/ci.yml`](../.github/workflows/ci.yml), [`scripts/check-coverage.py`](../scripts/check-coverage.py), [`scripts/check-licenses.py`](../scripts/check-licenses.py) | `tests/test_repo_gates.py`, `tests/test_repo_artifacts.py` |
