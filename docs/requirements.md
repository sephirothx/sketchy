...
| **R-AVA-06** | A registered player MAY instead wear a **doodle**: one of this deployment's own drawings, chosen by name and stored as the name (`doodle:<name>`), never as bytes. It is drawn in the disc's ink from the sprite the frontend ships, so the name colour still shows through and keeps its meaning. The server MUST refuse a name it does not have, and MUST NOT apply the moderator's upload block to a doodle — the block exists to stop a removed picture coming straight back, which a drawing of ours cannot be. One picture per account still holds: a doodle replaces an upload and an upload replaces a doodle. Claiming an account assigns a random doodle, so a registered player looks claimed from their first seat; guests get none (R-AVA-02). The sprite is generated output, not artwork checked in by hand — the drawing is the centrelines in [`scripts/brand/avatar-doodles.mjs`](../scripts/brand/avatar-doodles.mjs), swept with the brand's brush by [`build-avatar-doodles.mjs`](../scripts/brand/build-avatar-doodles.mjs), and a hand edit to the sprite is one the next build silently discards. The server's list, the client's list and the sprite's symbols MUST agree, and a test holds them together. [`auth/avatar_doodles.py`](../backend/app/auth/avatar_doodles.py) |
