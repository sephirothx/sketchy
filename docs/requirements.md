...
| **R-UX-01** | A room MUST lay itself out to the viewport rather than flowing beneath a banner — on a desktop as much as on a phone. From 901px wide and 640px tall the room is pinned to the window and its three columns scroll inside themselves; below that floor it falls back to the ordinary scrolling page, because columns a few rows tall are worse than one page that scrolls. Nothing in a room is a reading surface: the roster and the guesses are live lists, and scrolling one of them must not carry the drawing off the screen. [`styles/game-room.css`](../frontend/src/styles/game-room.css) |
...
| **R-UX-06** | The drawing MUST NOT be presented wider than **1180px**, whatever the window. Strokes travel as normalised coordinates and are replayed onto a backing store fixed at 800 × 600 — fixed because `draw_fill` is pixel-addressed against that grid and validated against it on the wire, so moving the store would land the same flood fill on different pixels for different clients. Presenting the element larger is therefore a pure upscale; 1180px is 1.48×, and past it crayon-weight strokes visibly soften, so the remaining width goes to the side columns rather than to a blurrier drawing. [`styles/canvas.css`](../frontend/src/styles/canvas.css), [`lib/canvasHistory.ts`](../frontend/src/lib/canvasHistory.ts) |
| **R-UX-07** | A page MUST spend the width the window has: the shell steps **1240 → 1600 → 1960** at 1500px and 2100px, and the room's side columns (250/300 → 290/340 → 320/380) and the canvas cap (760 → 1000 → 1180) step with it. Running text keeps a measure whatever the shell does, so widening a page never turns a paragraph into one long line. [`styles/layout-primitives.css`](../frontend/src/styles/layout-primitives.css) |
...
| Layout shell, room viewport pinning, canvas presentation cap | [`styles/layout-primitives.css`](../frontend/src/styles/layout-primitives.css), [`styles/game-room.css`](../frontend/src/styles/game-room.css), [`styles/canvas.css`](../frontend/src/styles/canvas.css) | `tests/e2e/test_desktop_layout.py` |
