...
| **R-AUTH-17** | A signed-in password change (`POST /api/auth/password/change`) MUST prove the current password first, MUST hold the same password rules a reset does, and MUST end the same way R-AUTH-10 requires: every session revoked, the caller signed back in. Changing a password is how somebody evicts a device they no longer trust, so a change that left other sessions standing would be the failure worth avoiding. A guest MUST be refused — there is no password to change — and the reset link stays the only route for somebody who does not know theirs. |
...
| # | Requirement |
| --- | --- |
| **R-MOD-01** | Any signed-in player, **including a guest account**, MUST be able to submit a private report. Reports use one of six bounded reasons (harassment, offensive drawing, inappropriate name, cheating, spam, inappropriate avatar) plus ≤ 2000 characters of detail and an optional ≤ 32 KiB JSON context snapshot. |
...
| **R-UX-06** | Every screen's header MUST be the one bar, in three slots: **where you are** — the wordmark, then either a page name or the room chip; **what is going on** — the room's phase, empty outside a room; and **you** — at most three controls, of which the last two never move (**Player settings**, then the identity chip). No page composes a bar of its own. The one remaining slot is the place's single action: **Create room** in the lobby, the **Room menu** in a room, nothing on a sub-page. A phone keeps the same three slots with one control each. The cap is the point: an unlabelled seventh icon is not a control anyone can find, and a bar that is identical on every sub-page never says which page this is. [`components/AppHeader.tsx`](../frontend/src/components/AppHeader.tsx) |
| **R-UX-07** | The wordmark MUST be the way back to the lobby on every screen but a room, where a click MUST NOT silently give up the seat, and every sub-page MUST carry a back control on both devices — back in history when the previous entry was one of this application's, to the lobby otherwise. |
| **R-UX-08** | A room's actions beyond the bar MUST live in one **Room menu**, whose rows and order are identical on a desktop dropdown and a phone sheet: the invite first, the routine actions next, and **Leave the room** last, separated, and styled as destructive. The phone's copy also carries what its bar has no room for (**Players and scores**, **Player settings**). [`components/RoomMenuSheet.tsx`](../frontend/src/components/RoomMenuSheet.tsx) |
| **R-UX-09** | One room MUST be described the same way everywhere it is described. One list decides which facts those are, in which order, and in what words — players, rounds, drawing time, scoring, hints, drawing rules — and every surface renders **a prefix of it**, never a selection of its own. The invite page and the waiting room show all six; the lobby card shows the first three, because a row per open room has no space for six and showing them pushed the next room off a phone (the other three are one tap away, behind Join). The waiting room MUST show them to everyone, not only to the host, with the host's way to edit them beside them. [`lib/roomFacts.ts`](../frontend/src/lib/roomFacts.ts) |
| **R-UX-10** | The waiting room's invite MUST offer the code **and** a QR code of the same link beside it, so the phone across the table can join by camera rather than by typing six characters. Both MUST encode the address the Share and Copy controls hand out; they cannot be allowed to name different rooms. |
...
| One header, the Room menu, the room described one way | [`components/AppHeader.tsx`](../frontend/src/components/AppHeader.tsx), [`components/RoomMenuSheet.tsx`](../frontend/src/components/RoomMenuSheet.tsx), [`lib/roomFacts.ts`](../frontend/src/lib/roomFacts.ts) | `frontend/tests/roomFacts.test.mjs`, `tests/e2e/test_waiting_room.py`, `tests/e2e/test_disruptive_actions.py`, `tests/e2e/test_restart_vote.py`, `tests/e2e/test_player_afk_reconnect.py`, `tests/e2e/test_a11y_core_flows.py` |
