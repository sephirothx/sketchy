...
| # | Requirement |
| --- | --- |
| **R-MOD-01** | Any signed-in player, **including a guest account**, MUST be able to submit a private report. Reports use one of six bounded reasons (harassment, offensive drawing, inappropriate name, cheating, spam, inappropriate avatar) plus ≤ 2000 characters of detail and an optional ≤ 32 KiB JSON context snapshot. Where the **server** selects the evidence (`report_player`, R-MOD-03), the detail MUST be optional: the messages and the drawing are usually the whole complaint, and a reporter mid-turn is not made to write it out. |
