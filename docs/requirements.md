...
| **R-UX-09** | One room MUST be described the same way everywhere it is described: the same six facts, in the same order, on the lobby card, the invite page, and the waiting room. The waiting room MUST show them to everyone, not only to the host, with the host's way to edit them beside them. |
...
| One header, the Room menu, the room described one way | [`components/AppHeader.tsx`](../frontend/src/components/AppHeader.tsx), [`components/RoomMenuSheet.tsx`](../frontend/src/components/RoomMenuSheet.tsx), [`components/WaitingRoomPanel.tsx`](../frontend/src/components/WaitingRoomPanel.tsx) | `tests/e2e/test_waiting_room.py`, `tests/e2e/test_disruptive_actions.py`, `tests/e2e/test_restart_vote.py`, `tests/e2e/test_player_afk_reconnect.py`, `tests/e2e/test_a11y_core_flows.py` |
