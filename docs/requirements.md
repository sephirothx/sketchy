...
| **R-AVA-01** | A registered player MAY upload a picture to stand in for their initial. The browser MUST crop it to a centred square and re-encode it to a 256×256 PNG before sending; the server MUST accept only a PNG of exactly that size under 128 KiB, checked from the fixed-position header without decoding the image, and MUST serve it only ever as `image/png` with sniffing disabled. That is the whole surface a modified client has: any valid PNG up to the cap, shown as an image and nothing else. [`auth/avatars.py`](../backend/app/auth/avatars.py) |
