...
the window's 16px gutter when that is nearer. It takes the reach only while the
column is at its full width: below that the column already fills the window,
and `100vw` counts a classic scrollbar the column does not have. Rules is a
reading page at 928px; its width is stated as `.ops-page.rules-page`, because
`operator.css` arrives in the page's own chunk after the entry sheet, and a lone
`.rules-page` lost that tie and ran to the shell. The wordmark is a link, not a
heading: each page's one `<h1>` is its own name (the lobby's is visually
hidden), and `useDocumentTitle` names the tab after it — "Gallery · Sketchy",
a room's own name in a room, the brand alone on the lobby.
