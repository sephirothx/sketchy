...
Routes ([`frontend/src/App.tsx:72`](../frontend/src/App.tsx)):
...
| `/settings`, `/settings/:section` | [`SettingsOverlay`](../frontend/src/components/SettingsOverlay.tsx) — drawn *over* the page it was opened from, so a live room is never unmounted (R-SET-06). The route element is `null`; the overlay renders it. |
