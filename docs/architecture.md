...
│   └── useCanvasPointerInput.ts   Pointer → drawing action translation; thins samples (lib/pointThinning.ts)
├── store/
...
4. **One dialog anatomy** ([`components/ui/ModalShell.tsx`](../frontend/src/components/ui/ModalShell.tsx), R-A11Y-04).
   ...
   new dialog starts from `ModalShell`, not from a fresh `.modal-overlay`. Every
   moderation report goes one step further and is one dialog,
   [`ReportDialog`](../frontend/src/components/ReportDialog.tsx): the five report
   components (a seat, an account, a lobby line, a Gallery drawing, prompt content) say
   only their target, reasons and what the server attaches, while first focus, the
   optional words under one limit (R-MOD-01), the counter and the in-dialog *Report sent*
   with one Close are decided there once. The bug report is not one of them (R-BUG-02).
...
actually about: the flush interval is the largest lever on drawing bandwidth, and the
only way to judge it is to watch somebody draw while it moves — so the change re-arms
the drawer's timer mid-stroke rather than waiting for their next turn. The number the
byte curve pointed at used to be the wrong one because a viewer painted each batch
at once; since #559 a viewer plays a batch out over the interval that follows it
([`lib/strokePlayback.ts`](../frontend/src/lib/strokePlayback.ts)), presentation
decoupled from the wire while the history and commits stay synchronous, and the
default is 80 ms.
...
### Version skew, and what a deploy assumes

Both ends deploy together, from one origin, so the only client that ever speaks an older
contract is a tab that was open across the deploy. The handshake compares versions and
tells such a socket to reload (wire §1); since #476 it is also held to that: the
dispatch door in [`handlers/context.py`](../backend/app/handlers/context.py) refuses every
command from a quarantined socket before parsing, and the socket is closed after five
seconds if it has not gone. Every HTTP response carries the same number in
`X-Sketchy-Protocol`, so a tab that is not on a socket is caught by its next request. The
client reloads at most once per server version and, if that did not help, stops
reconnecting and shows an *update required* banner with a reload of the player's choosing
([`lib/protocol.ts`](../frontend/src/lib/protocol.ts),
[`lib/updateRequired.ts`](../frontend/src/lib/updateRequired.ts)).

What that lets a deploy assume: **no two versions need to coexist.** There is no
`/api/v1`, no N/N−1 support and no second code path for an older client (requirements
N-13); a rollback is a deploy whose number is lower, and a tab reloads onto it the same
way. What it costs: a deploy that changes the socket or REST contract interrupts every
open tab once — a reload, which the planned-deploy drain (§6) already accounts for by
letting a running game finish first.
...
naming a series the server does not expose. The dashboards follow the same rule
...
guessers: guess          (volatile, acknowledged; a retry carrying a seen id stops here)
  └─ handlers/chat.py  →  Game.submit_guess
       ├─ correct  → emit correct_guess (room), with the server's guess time;
       │             the receipt and the guesser's own line ride the
...
| [`app/announcements.py`](../backend/app/announcements.py) | What the room says about itself, as facts rather than as English sentences. |
...
| [`app/message_limits.py`](../backend/app/message_limits.py) | Shared backend limits for player-authored text: chat, guesses and report details. |
...
| `frontend/src/store/` | `authStore.ts`, `canvasBudgetStore.ts`, `emailStateStore.ts`, `friendInviteStore.ts`, `friendsStore.ts`, `gameStore.ts`, `lobbyChatStore.ts`, `presenceStore.ts`, `roomEntryStore.ts`, `roomsStore.ts`, `serverNoticesStore.ts`, `settingsMigrations.ts`, `settingsStore.ts` |
| `frontend/src/hooks/` | `useBottomDock.ts`, `useCanvasPointerInput.ts`, `useCanvasProtocol.ts`, `useDocumentTitle.ts`, `useEmailStateSync.ts`, `useFocusTrap.ts`, `useFriendInviteAnswer.ts`, `useGameSocketListeners.ts`, `useLobbyChannel.ts`, `useMediaQuery.ts`, `useNameField.ts`, `useRoomEntry.ts`, `useRoomHistory.ts`, `useRoomSessionReconnect.ts`, `useScratchPadProtocol.ts`, `useServerNotices.ts`, `useSettingsRoute.ts`, `useToolbarLayout.ts`, `useToolbarState.ts`, `useVisualViewportCssVars.ts` |
...
What the surfaces share is not restated in each either: the scales — four radii, two
motion durations, one disabled opacity — are tokens in
...
[`styleScales.test.mjs`](../frontend/tests/styleScales.test.mjs) fails on a value off a
scale, on a weight [`main.tsx`](../frontend/src/main.tsx) does not load (the browser
...
A media query cannot read a custom property, so the breakpoints are a fixed set of
widths rather than tokens, each named as the first width of the wider side — 481, 641,
721, 901, 1001, 1200, 1500 and 2100 as a `min-width`, one less as a `max-width`, so no
width is on both sides. Components asking through `useMediaQuery` use the same numbers.
They are listed in
...
A list with nothing in it says so through one component,
[`components/ui/EmptyState.tsx`](../frontend/src/components/ui/EmptyState.tsx) — a
dashed outline, a title, an optional line and at most one control, with a compact size
for a list inside a panel — and a wait with no skeleton to draw is one `.loading-note`
line (`styles/primitives.css`). There were four empty treatments and three loading ones
on neighbouring pages, so the same situation read as different news on each.
...
`min(var(--vv-height), 100dvh)` less `--banner-height`, taking whichever of the two viewport signals is
...
[`useServerNotices`](../frontend/src/hooks/useServerNotices.ts) records from the
socket. The same facts decide the room's stage (`roomStage`): an outage that outlasts
`STAGE_PAUSE_DELAY_MS` pauses it — the shell's regions become `inert` under a card, and
the header, which is outside the shell, stays usable — and a rejoin refused with
`room_not_found` or `room_ended` replaces it with an end card (#823). That refusal is
recorded once per room code and stops the rebind loop, because the reason is read from
...
phone page's docked controls rather than on them (in a room the invite is not there at
all but a chip in the header, which claims it from the card through
[`friendInviteStore`](../frontend/src/store/friendInviteStore.ts), #1176): each dock publishes how far it reaches
...
The header is the exception to per-page widths: `AppHeader` spans the shell on
every page, so the wordmark and the identity chip sit in the same place whether
the page below is the 928px lobby, the 1008px Gallery or a 1240px library page.
Each page declares its content column as `--page-width` beside its max-width,
and `.lobby-header` (in
...
(26px), on every page and at every width, with one exception: the invite page
on a phone (600px and narrower) keeps 18px, because it is the one screen a
first-time visitor lands on from a link and its Join button has to clear the
fold. A layout that puts a gap of its own under the bar - the pinned lobby's
flex gap - subtracts it from the bar's margin rather than adding to it. Rules is a reading page at 928px; its width is stated as
...
its own name (the lobby's is visually hidden; a page that is only an empty
state makes that state's title the heading), while the staff queues head the
case that is open. `useDocumentTitle` names the tab after the page rather than
