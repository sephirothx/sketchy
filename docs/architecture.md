...
process, and twenty seating joins per socket per minute. A socket past the ceiling is
...
because a count is only ever as right as the last event that moved it.
