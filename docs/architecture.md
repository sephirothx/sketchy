...
(`get_seat_account`), and stamps `last_active_at` after the acknowledgement instead of
before it, on a flat hang guard rather than the entry's deadline (which the task would
