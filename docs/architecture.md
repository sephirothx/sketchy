...
| [`app/auth/avatar_doodles.py`](../backend/app/auth/avatar_doodles.py) | The doodle set a registered player may wear instead of an initial (R-AVA-06). |
...
The avatar doodles (R-AVA-06) come down a pipe of their own, because there was no artwork to derive them from. `scripts/brand/avatar-doodles.mjs` holds each doodle as centrelines on the same 24×24 grid the UI icons use, and `node scripts/brand/build-avatar-doodles.mjs` sweeps a brush along them into `frontend/public/avatars/doodles.svg`: a filled outline with a tapering, breathing width, which is the same kind of mark the Inkscape-authored logo and 404 drawing are made of. Putting the hand in the generator rather than in the drawings is what keeps it consistent — a doodle only has to be the right shape, and the wobble, the pressure, the round tips and the lap where a closed line passes its own start are the brush's. Everything it jitters is seeded on the doodle's id, so a rerun rewrites the file byte-for-byte and `scripts/check-doodles-regenerated.sh` can tell a hand edit from a redraw. Each symbol's `viewBox` is a square on the circle that contains its own ink, not on its bounding box, so 80% of the disc means the same for the whale as for the bear and nothing pokes through the rim; the paint is a presentation attribute rather than a stylesheet, because a stylesheet does not follow a symbol through an external `<use>`.
