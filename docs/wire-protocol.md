...
| `POST` | `/api/auth/second-factor/confirm-owner` | `{ password }`. Records that the factor already in place is the account owner's — what a staff role requires and setting one up does not ask for |
