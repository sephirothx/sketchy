...
| `GET` | `/api/users/{user_id}/pins` | The player's **Pinned drawings** for anyone signed in, a guest included (R-PIN-06); no session and no such player are the same `404`. Each entry is `{turnId, roundNumber, turnNumber, drawerDisplayName, drawerNameColor, drawerIsAnonymous, prompt, strokeCount, reactions: [{seatId, emoji}]}` — the game-detail turn's shape where the two overlap, credited through the frozen drawer snapshot, and **no `gameId`** (R-PIN-07). A pin whose drawing is no longer ready is left out, not shown as a hole |
