...
a conditional request is answered against the bytes the client would receive — the copy is chosen before the validators are compared, so a 304 carries that copy's `ETag` and `Vary: Accept-Encoding`, never the identity file's (#978). A copy is served only when it is a regular file with a single name: a symlink, a hard link or a directory wearing the `.br` name is ignored, and a copy asked for under its own name is a 404 whatever case the suffix is written in. Three things this deliberately does not do: a copy is not checked against its file's modification time (the build writes both in one step, and a mismatched pair is a broken build, not a request-time question), `Accept-Encoding: *` is read as naming no coding rather than all of them, and `identity;q=0` is not honoured - a client that refuses the stored bytes and accepts no coding is still served the stored bytes. Nothing
