...
| `select_prompt` | `SelectPromptPayload` — `{index}`, the offer's position in `your_prompt_choices` (0–2). By position rather than by text (#1181): the text is one language's spelling of the offer, and a room whose seats read different languages (#1182) offers one prompt spelled several ways. An index past the offers answers `prompt_unavailable` | ✓ | [`game.py`](../backend/app/handlers/game.py) |
...
| `your_prompt_choices` | `{choices: string[], seconds}` — the offers as the drawer reads them; `select_prompt` answers with a position in this list | drawer only |
...
- Only the frame that **starts** an action carries `[generation, sequence, nonce]`. Path
  ...
  required: an identical opener — a dot tapped twice on one spot — is no evidence
  either way, so an identity without a nonce is refused.
...
| `POST` | `/api/auth/password/reset` | Revokes every session, then signs the user in — `{ok, signedIn, reason?}`. A **staff** account is not signed in (`signedIn: false`): a reset proves the mailbox and R-AUTH-20 wants the code too, so it goes through login (#996). Nor is a **suspended** account: the password takes, and login answers the suspension (#1052). `reason` (`second_factor` \| `suspended`) accompanies `signedIn: false` so the page can say which. (`AUTH_RESET_PERFORM_LIMIT`) |
...
| --- | --- | --- |
| `GET` | `/api/gallery` | One page of the **Gallery** (R-GAL-01..04): every `ready` drawing from a **public** game. `sort` = `hot` \| `new` \| `top` (default `hot`), `window` = `all` \| `month` \| `week` (default `all`, read by `top` only), `limit` ≤ 24, opaque `cursor` (keyset — where the last row stood, so a game finishing between two reads never makes a page repeat a drawing, #1072; signed by the process and bound to its order, so a mangled, forged, foreign or pre-restart one reads as page one). Hot and Top rank on live reaction facts, so a reaction landing between two pages can carry a row across the cut: the client drops a drawing it already shows, and a row moved the other way waits for a reload (R-GAL-04); New pages exactly. Paging **stops 480 rows deep** with an empty page, the community catalogue's rule. Hot looks back fourteen days. Any session, a guest's included; **no session is `403 account_required`** with `params.action` = `gallery` (R-GAL-02) — refused, not an empty page, so the client can say why. An unknown sort or window is `422 unknown_sort` naming the field. Each entry is `{turnId, roundNumber, turnNumber, drawerDisplayName, drawerNameColor, drawerIsAnonymous, prompt, strokeCount, finishedAt, reactionCounts: {code: n}, myReaction, drawnByMe}` — the pin's shape plus the finish time and the viewer's own facts, and **no `gameId`**, room name or reactor's name (R-GAL-03). Answers `{entries, nextCursor}` |
...
| `PATCH` | `/api/moderation/reports/{report_id}` | moderator+ | `403` when the incident is about the caller and they are not an administrator (R-MOD-07, #1003, #1063). Decides the **whole incident** the named report belongs to and answers with it: one note, one step-up, one action, every report of it resolved with its own reviewer, moment and audit entry (R-MOD-17). Any report of the incident reaches it. Review is one-way, so a report another moderator already decided answers `409` and nothing is written |
| `GET` | `/api/moderation/prompt-lists` | `{lists, waiting}` — publications the review switch is holding (R-LIST-13): under review **and public**, not a moderator's own (R-MOD-07, #1063), oldest first. **Not a report queue**: nothing was complained about, so a row carries the list, its owner, its `version` and how long it has waited, and no reporter, reason or evidence |
...
| `PATCH` | `/api/moderation/prompt-content-reports/{report_id}` | moderator+ | `403` for a moderator's report about their own list (R-MOD-07, #1063). A resolution chooses Active or Hidden; a dismissal cannot mutate content. Decides the **whole incident** — every pending complaint about that target — and answers with it: the content is hidden or left up once, under one note (R-MOD-10, R-MOD-17). `409` if any of it was already decided |
