...
| `POST` | `/api/auth/second-factor/confirm` | `{ secret, code, password? }` → `{ ok, recoveryCodes }`. The password is **optional** when setting one up (giving it records the proof a staff role needs) and **required** when replacing one, which destroys a credential (R-AUTH-20). The ten codes appear **here and nowhere else**: only their SHA-256 hashes are kept. Re-enrolling replaces the secret and every code issued against the old one |
| `POST` | `/api/auth/second-factor/confirm-owner` | `{ password, code }`. Records that the factor already in place is the account owner's — what a staff role requires and setting one up does not ask for. Both: the password says the owner is asking, the code says the enrolled authenticator is theirs. The code is spent like any other, so one just used to enrol is refused until the next step |
