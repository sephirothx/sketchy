...
| `PUT` | `/api/users/me/avatar/doodle` | Wear one of the deployment's own drawings (R-AVA-06). Takes `{ name }`; `400` for a name the server does not have, `403` for a guest. No rate limit and no upload block: nothing is stored but the name. Answers `{ avatarKey, avatarUrl }`, the URL pointing into the frontend's sprite (`/avatars/doodles.svg#fox`), which is how the client draws it in the disc's ink |
