...
| `GET` | `/api/prompt-lists/community` | `{lists, nextCursor}` — published lists (R-LIST-14). Filters: `language`, repeated `tag` (a list must carry **every** tag given, not any of them), `sort` = `stars` \| `newest`, `limit` ≤ 48, opaque `cursor`. Open to a signed-out caller, whose rows carry `starredByMe: null` — a different answer from `false`. A row names its owner's **display name and no account id**: a stable third-party identifier in a public listing is a join key for anybody who collects the pages |
...
| `GET` | `/api/prompt-lists/mine` | The caller's own lists, each with the `tags` its current revision carries and the `starCount` it has been given — **the number only**: who starred a list is disclosed to nobody, its owner included |
...
| `GET` | `/api/moderation/prompt-lists` | `{lists, waiting}` — publications the review switch held back (R-LIST-13), oldest first. **Not a report queue**: nothing was complained about, so a row carries the list, its owner and how long it has waited, and no reporter, reason or evidence |
| `PATCH` | `/api/moderation/prompt-lists/{prompt_list_id}` | `{state: active \| hidden, note}`. Releases a held list into the catalogue or takes it down; **409** for a list nobody held, so a decision cannot be made about content this queue never showed. Hiding tells the owner, as a takedown from a report does. Writes `prompt_list.review_active` / `prompt_list.review_hidden` |
