...
| `POST` | `/api/auth/second-factor/confirm` | `{ secret, code, password }` → `{ ok, recoveryCodes }`. The password is required for a first enrolment as much as a replacement (R-AUTH-20). The ten codes appear **here and nowhere else**: only their SHA-256 hashes are kept. Re-enrolling replaces the secret and every code issued against the old one |
