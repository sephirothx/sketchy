...
| `POST` | `/api/auth/passkeys/verify` | `{ credential }` → `{ ok, user, steppedUp }`. One act, two uses: a caller already holding a session on the account that signed is **stepped up** (R-AUTH-21); anybody else is **signed in**, with a session at the staff lifetime and the step-up recorded — the assertion is that proof and it is one request old |
