...
([`backend/app/main.py`](../backend/app/main.py), `SPAStaticFiles`): the build's own Brotli or
gzip copy of a file when the browser accepts one (`Content-Encoding` set, `Vary: Accept-Encoding`
on the 200 and on every 304, the copy's own `ETag`, #978), gzip at level 4 for other eligible
responses and never for a response whose type is already compressed (`image/`, `font/`,
`audio/`, `video/`, archives), `Accept-Encoding` read with its weights so `q=0` refuses, Vite's fingerprinted `/assets/` served `immutable` with a one-year lifetime,
