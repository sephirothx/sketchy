...
| `GET` | `/api/moderation/gallery` | moderator+ | `{review, waiting, candidates}` — This week's review queue (R-GAL-10), leaving out a moderator's own drawings (R-MOD-07, #1063): whether `gallery.shelf_review` is set, and while it is, the current Top-week drawings nobody has decided (the six This week would take and six behind them — fewer for a moderator, whose own are left out), each in the Gallery entry's shape. With the switch off nothing waits: the shelf is Top-week directly |
