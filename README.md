...
- Real-time synced canvas (freehand brush + rectangle/ellipse/triangle shape tools). A triangle's base lies on the row where the drag started, so the starting point is always a corner, and it points toward where the drag ended: drag downwards and it is drawn upside down. A brush stroke is thinned as it is drawn: samples that would move the line by less than a quarter of a pixel are not sent, with the error bounded for the whole stroke, and the drawer's own canvas is painted from the same samples the viewers get, so everyone rasterizes one line. Points go out every 80 ms, each frame relative to the last point sent, the last batch of a stroke carrying its end, and a viewer plays each batch out over the next 80 ms at the screen's own rate rather than painting it in one step, so it sees ink smoothly, up to 80 ms behind the drawer's hand.
