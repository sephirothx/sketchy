...
- **Canvas blocked**. The canvas is painted by reading its pixels back, and a browser that answers such a read with other pixels than it painted - Firefox's `privacy.resistFingerprinting`, the default in Tor Browser, Mullvad Browser and LibreWolf - drew every stroke in a rectangle of static. A page cannot read or change browser settings, so the game checks what a read returns on arrival and, where it is scrambled, says so: a banner in the lobby and a chip in a room, telling the player to allow the site's canvas data or turn the setting off, with **Check again**, which repaints the canvas once reads are true. Brave also perturbs reads, by one step on a channel that nobody can see; it is not flagged.
