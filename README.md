...
concept only when the checked-in files deliberately repeat that ID. Each
language's **Standard** list repeats the English ones deliberately - `anchor`
and `Anker` are one concept in two languages - while its **Extended** list is
written natively and carries concepts of its own. Official answers carry no
...
frontend/
  ...
  src/
    ...
    lib/brushSizes.ts The sizes the brush slider stops at, and the default among them
    lib/pathWidths.ts A path whose width changes along it: the ramps between its keyframes, as runs of one width (#828)
    lib/penPressure.ts A pen's pressure as a brush width: the ceiling, the 2 px floor, by ratio, which pens are believed
    lib/widthKeyframes.ts Which of a pen stroke's widths are sent: keyframes, within a tolerance of the curve
...
# A mid-turn entry and a reconnect: time to the drawing, and the canvas bytes each costs (#877)
# (a diagnostics build and a server on localhost:8000, as the E2E suite uses; see the script)
...
backend/.venv/bin/python benchmarks/path_widths.py --brush 6 12 32 --tolerance 0.1 0.15
...
puts the width on the wire four ways: keyframes inside the frame being sent anyway (what
is implemented), a byte on every point, a frame per keyframe, and the six stepped levels
keyframes replaced. `--smoothing 0 --tolerance 0.15` measures it as it was before the
pressure was smoothed and the tolerance widened with the line.
...
them. How many keyframes a stroke needs is what decides the result. A repeated identical batch, which is what the benchmark
...
interaction-handler time, canvas readback calls/time/pixels (zero for the drawing canvas, which is never read), heap deltas, long
...
The dashboards are in [`ops/grafana/`](ops/grafana/) (#968): Overview, Connections,
Database and Storage, **generated** by [`generate.py`](ops/grafana/generate.py) and
committed as JSON, with provisioning for the Prometheus datasource and the dashboards.
...
The database's own statistics come from
...
`sketchy_probe_step_seconds{step}` to the textfile, plus
`sketchy_probe_last_run_timestamp_seconds` so a stale file pages rather than
reporting last hour's success. Its two guest sessions are kept
...
- The header is the same on every page: it spans the widest column the window allows,
  ...
  Rules, while each page keeps the width its own content reads best at. From 901px it
  also links the site's pages, with the one you are on marked. The browser tab names the
