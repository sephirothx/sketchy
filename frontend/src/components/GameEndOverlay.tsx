...
export function GameEndOverlay({
  ...
  return <main className="game-end-overlay" aria-labelledby="game-end-title" aria-live="polite" data-testid="game-end-overlay">
    <section className="game-end-podium">
      ...
      <h1 id="game-end-title">{scoringMode === "default" ? <><span className={playerNameClass(scores[0]?.isAnonymous)} style={playerNameStyle(scores[0]?.nameColor, scores[0]?.isAnonymous)}>{scores[0]?.nickname ?? "The room"}</span> takes the crown!</> : "A great round of drawing"}</h1>
