...
const REFUSALS: Record<ErrorCode, Sentence> = {
  ...
  server_busy: "Sketchy heeft het even te druk. Probeer het zo meteen opnieuw.",
...
export const NL: Catalogue = {
  ...
  friendsOverlay: {
    ...
    createAnAccountOrSignIn: "Maak een account aan of log in om vrienden toe te voegen.",
    loading: "Laden…",
  ...
  communityCataloguePage: {
    ...
    listsPlayersPublished: "Lijsten die spelers hebben gepubliceerd.",
    couldNotLoadTheCatalogue: "De communitycatalogus kon niet geladen worden.",
    couldNotOpenThatList: "Deze lijst kon niet geopend worden.",
    language: "Taal",
    sortBy: "Sorteren op",
    mostStarred: "Meeste sterren",
    newest: "Nieuwste",
    onlyOnesIStarred: "Alleen met mijn ster",
    clearFilters: "Filters wissen",
    ...
    copiedFrom: "Gekopieerd uit {list} van {owner}",
    ...
    chooseAListBody: "De labels en alle woorden erin.",
    promptsHeading: "Woorden",
    inTheAuthorsOrder: "In de oorspronkelijke volgorde",
    shownOfTotal: (p: { shown: number; total: number }) => `${number(p.shown)} van ${number(p.total)} getoond`,
    exploreAll: (p: { count: number }) => `Alle ${counted(p.count, { one: "woord", other: "woorden" })} bekijken`,
    allPrompts: (p: { count: number }) => `Alle ${counted(p.count, { one: "woord", other: "woorden" })}`,
    searchPrompts: "Woorden doorzoeken",
    matchesOfTotal: (p: { matches: number; total: number }) => `${number(p.matches)} van ${number(p.total)}`,
    order: "Volgorde",
    authorsOrder: "Oorspronkelijk",
    alphabetical: "A–Z",
  ...
  galleryPage: {
    ...
    pause: "Pauze",
    play: "Afspelen",
    replayPosition: "Stand van het afspelen",
  ...
  profilePage: {
    ...
    statisticsAppearAfterFirstGame: "Statistieken verschijnen na het eerste afgeronde spel.",
    ...
    openPinnedDrawing: (p: { prompt: string; drawer: string }) => `${p.prompt} van ${p.drawer} openen`,
    moveLeft: "Naar links",
    moveRight: "Naar rechts",
    unpin: "Losmaken",
    couldNotUpdatePinnedDrawings: "Je vastgezette tekeningen konden niet worden bijgewerkt. Probeer het opnieuw.",
    couldNotLoadThisDrawing: "Deze tekening kon niet worden geladen.",
  ...
  settingsOverlay: {
    ...
    picture: "Afbeelding",
    pickDoodle: "Een tekening kiezen",
    uploadPicture: "Een afbeelding uploaden",
    removePicture: "Afbeelding verwijderen",
    removeDoodle: "Tekening verwijderen",
