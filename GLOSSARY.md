...
| **Player settings** | A player's own preferences, which travel with them between rooms. Reached at `/settings/<section>` from the first row of the identity chip's menu (in a room, also the **Room menu**'s Settings row), drawn over whatever page they were on, in four sections: **Account**, **Appearance**, **Sound & effects**, **Shortcuts**. Every row applies as it changes; only the rows a server can refuse keep a button. Not to be confused with **Room rules**, which belong to a room and are the host's. | user settings, profile settings, room settings |
| **Room menu** | Everything a player can do to the room they are in, other than play: copy the invite link (with the room's code), go **AFK** or come back, **Save image**, propose starting over, open **Player settings**, and *Leave room*, always last and in red. One list, dropped down from *Room* in the desktop bar and shown as the ⋯ sheet on a phone, in the same order; a phone adds *Players and scores*, because its players column is folded away. | room actions, overflow menu, kebab menu, ⋯ menu (in UI copy) |
