...
| **Runtime observation** | One thing the server recorded about its own behaviour - a join, a disconnect, a timer overrun, a stored drawing's size, a caller held to a command budget. Kept in raw form for a bounded window and in daily totals for ever. | metric, log line, telemetry |
...
| **Canvas blocked** | The notice for a browser that answers the game's canvas reads with other pixels than it painted - a privacy protection such as Firefox's `privacy.resistFingerprinting` - which would draw every stroke in static. A banner in the lobby and a chip in a room, with **Check again**. Named for what the player sees, not for the setting that causes it. | fingerprinting error, canvas error, broken canvas, scrambled canvas |
