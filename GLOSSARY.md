...
| **Recovery address** | The confirmed email address an account can be recovered through. An address that has been offered but not confirmed is not one, and cannot be sent a reset link. | email, contact address, backup email |
...
| **Prompt language** | The BCP-47 content language a room is **declared** to be in, chosen by its host at creation and fixed for the room's life. It selects the game's answer-matching rules — for the room's own quick prompts as much as for list content — and every selected **Prompt list** must be in it. It is distinct from the interface locale used for translated catalogue copy. | locale, UI language, list language, room language |
...
| **Lobby** | The browsable list of public rooms, with the lobby chat and the list of who is online beside it. On a phone a row of links at its foot leads to the **Gallery** (once there is a session), the **Community catalogue**, **Prompt stats** and the **Rules**; on a desktop the header carries the same links on every page. **The lobby is one place, and it is not inside a room.** | browser, room list, home |
...
| **Room menu** | Everything a player can do to the room they are in, other than play: copy the invite link (with the room's code), go away or come back, save the drawing, propose starting over, open **Player settings**, and **Leave**, always last and in red. One list, dropped down from *Room* in the desktop bar and shown as the ⋯ sheet on a phone, in the same order; a phone adds *Players and scores*, because its players column is folded away. | room actions, overflow menu, kebab menu, ⋯ menu (in UI copy) |
...
| **Kick vote** | A proposal to remove a player from the room. Once it passes, the player is out for the room's lifetime. | vote kick, vote-kick, boot |
