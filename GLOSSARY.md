...
| **Crash page** | What a screen shows when its own code throws — the not-found card again, with a bug drawn on the canvas. It offers a reload, the lobby, and a **Bug report** already filled with the error and the browser's own error tail, which the player may send with their own words. Distinct from the not-found page, which is a URL with nothing behind it. | error page, white screen, crash report, error boundary (in UI copy) |
