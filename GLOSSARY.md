...
| **Duplicate** | A second list of your own made from one of your own lists, from *My prompt lists*: private, with the same prompts and tags, and no history — it records no **Fork provenance**, counts as no **Copy** and credits nobody. Distinct from a **Copy**, which is always somebody else's **Published list**; a list that is itself a copy cannot be duplicated, so its credit cannot be dropped. | copy (for your own list), clone, fork |
