...
| **Doodle** | One of Sketchy's own drawings a registered player can wear in the disc instead of a **Picture**, picked by name from the pencil on the disc and drawn in their **Name color**. A new account is given one at random, so a claimed seat never wears the grey initial that marks a guest. Nothing is uploaded or stored but the name, so it needs no moderating. | preset avatar, default avatar, icon |
