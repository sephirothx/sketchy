...
| **Friends** | The screen holding every friendship and every request, sent or received, whether or not the other account is online. Reached from the account menu at `/friends`, drawn over whatever page it was opened on, so a request that arrives mid-game can be answered without leaving it. Not the same as **Lobby presence**, which answers who is around rather than who is a friend — that list says a request is waiting and stops there, because a row that comes and goes with presence is the wrong place to answer one. | friends list, contacts, social panel |
