...
| **Any language** | A **Prompt list** in no language at all — Pokémon, brands, places — stored as BCP-47 `zxx`. It is played in a room of any **Prompt language**, under that room's matching rules, and shows the Any-language mark (two speech bubbles) where other lists show a flag. Not a language a room can be opened in, and not the lobby filter's *Every language*, which means "don't filter". | language-neutral, universal, multilingual, no language, international |
