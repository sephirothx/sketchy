...
| **Incident** | One reported player, in one place: the **Reports** about them from a single room instance (or from the lobby) read and decided as one case. Derived from what each report records rather than assembled by hand, so it is what the queue shows and what one decision covers; the count of reporters is shown and never orders the queue. Distinct from a **Closed case**, which is an incident already decided. | case, ticket, cluster, pile-on |
