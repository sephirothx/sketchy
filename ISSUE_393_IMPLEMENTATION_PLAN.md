...
| #342 prompt content | `epic/342-prompt-content-usage` | In progress | #392 implemented stable concepts, immutable localized versions, scoped aliases/tags/editorial metadata, and alias-aware matching. #377 added the constrained language registry, homogeneous room resolution, localized catalogue copy, and language-aware UI. #376 added explicit UUIDv7 bundled identities, immutable content-hashed revisions, conflict-safe batched seeding, reword-safe counters, and game pinning. #398's locked partial scope now adds registered ownership, private/unlisted/public-reserved visibility, required unlisted share codes, exact revision fork provenance, structured revision tags, and actor/time-correlated Active/Under review/Hidden moderation fields. Public community discovery, favorites, and user-facing forking remain deliberately absent. Reversible migrations and focused constraint/repository/handler/contract tests pass; README and glossary updated; final child gate pending |
