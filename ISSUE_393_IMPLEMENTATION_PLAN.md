...
| #342 prompt content | `epic/342-prompt-content-usage` | In progress | #392 implemented stable concepts/versions and metadata; #377 added explicit language semantics/localized catalogue copy; #376 added UUIDv7 bundled identities, immutable revisions, conflict-safe seeding, and game pinning; #398 added the locked ownership/visibility/provenance/moderation schema while leaving discovery/favorites/user-facing forks absent. #379 locks quick custom prompts as explicit ephemeral room input and closes #330 durably: completed turns carry nullable source-version IDs, usage is authorized by exact pinned revision membership, and equal custom text cannot credit curated offers, picks, or guess totals. #318 provides registered owner create/edit/delete/reuse, bounded UUIDv7 immutable revisions, Private/Unlisted authorization, random share-code room selection without capability leakage, owner UI, and export/deletion lifecycle coverage. #378 adds post-moderation reports for an exact shared list or immutable prompt version, durable deletion-safe evidence, role-gated one-time Active/Hidden review with audit provenance, owner/player UI, reporter export coverage, filtering, and game-start revalidation without mutating already-pinned active games. #378 child gate: 652 backend tests (1 skipped), migration replay/model-drift checks, 158 frontend tests, 46 E2E tests, lint, and production build pass. #375 and the final umbrella gate remain pending. README and glossary updated. |
