...
| #341 game history | `epic/341-game-history-statistics` | In progress | #390 assigns the game UUIDv7 at runtime, reuses it for history and prompt-usage facts, stores a canonical SHA-256 payload digest, makes identical retries idempotent, and rejects different content under the same ID as an operator-visible conflict. #373 adds server-clock write times to configuration, prompt membership, game persistence, participant, turn, and guess facts; pre-migration times remain truthfully unknown, and persistence lag is now measurable separately from game event time. #369 adds explicit scoring/snapshot versions plus a canonical exact rule snapshot covering numeric score/hint parameters, drawer bonus, drawing tools/colors/time, prompt rules, and pinned source revisions; legacy rules stay version zero/unknown, participant detail and export retain the full snapshot, and public summaries expose safe typed/version fields. #368 records the actual post-shadowing pool as Curated/Custom/Mixed/Built-in fallback, normalizes exact game revision sources, and persists every ordered prompt offer with selected flag, immutable text/version identity, and all containing revisions; participant detail/export expose the evidence without share capabilities. #367 adds the selected turn's direct nullable immutable prompt-version FK plus source kind while retaining text, enforces agreement with the selected offer, and marks pre-provenance rows Legacy unknown. Its migration avoids SQLite parent-table rebuild/cascade data loss and proves legacy turns survive upgrade. #374 gives every factual runtime player a stable UUIDv7 history seat even without a cookie/account, preserves separate nullable-account participants, drawer and guess links plus frozen presentation, and keeps player/turn/guess counts truthful; legacy linked facts are backfilled without inventing identities for unknown rows. #389 freezes eligibility at drawing start and records every current/late non-drawer seat's eligibility reason, outcome, terminal state, correct time, per-seat attempt/hint facts, plus an optional linked correct-guess child; no-scoring awards remain zero, legacy migration backfills only provable correct outcomes, participant detail/UI and private export expose the facts without guess text. #387 locks display name, name color, and guest state as historical presentation while username/avatar remain live-profile-only; normal edits preserve snapshots/profile links, deletion tombstones them, and merged identities retain distinct factual seats. #386 adds ordered UUIDv7 score events for gross guess awards, hint charges, drawer bonuses, and append-only corrections with exact rule versions; repository validation matches gameplay events and reconciles every cached final score, no-scoring games emit no hypothetical events, and legacy games remain explicitly version zero. #372 retains accepted player-authored chat/wrong/correct text for 30 days under ordered UUIDv7 IDs, correlates it to runtime game/turn/seat IDs, records exact post-Block Room or Prompt-aware account audiences, and exposes no transcript API; reports may immutably pin up to 20 unexpired messages only when the target authored and reporter received them. Startup/hourly expiry, private export, account-erasure/tombstone behavior, and migration downgrade are covered. The current integrated gate passes 672 backend tests (one expected skip), 158 frontend tests, frontend lint/build, one Alembic head, and all 46 browser scenarios; README and glossary are current. |
