...
async def test_a_304_answered_before_a_copy_is_chosen_still_says_vary(built_app):
    """StaticFiles answers a conditional request against the original file
    itself, before any compressed copy is chosen. RFC 9110 §15.4.5: a 304
    repeats the `Vary` its 200 would have carried, or a cache that holds the
    Brotli body may hand it to a client that asked for none (#978 review)."""
    app, _, _ = built_app
    _, original, _ = await request(
    ...
    status, headers, _ = await request(
        app,
        "/assets/app-AbCdEf12.js",
        headers={"Accept-Encoding": "br", "If-Modified-Since": original["last-modified"]},
    )
