...
class PersistentRoom(Base):
    """Owner-controlled durable configuration; never a live-game snapshot."""

    __tablename__ = "persistent_rooms"
    __table_args__ = (
        _values_check(
            "scoring_mode", SCORING_MODES, "ck_persistent_rooms_scoring_mode"
        ),
        _values_check("hint_mode", HINT_MODES, "ck_persistent_rooms_hint_mode"),
        _values_check(
            "color_mode",
            ("all", "palette", "colorblind_safe", "black_and_white"),
            "ck_persistent_rooms_color_mode",
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "rounds >= 1 AND rounds <= 10", name="ck_persistent_rooms_rounds"
        ),
        CheckConstraint(
            "drawing_seconds IN (15, 30, 60, 90, 120, 180, 240, 300)",
            name="ck_persistent_rooms_drawing_seconds",
        ),
        CheckConstraint("version >= 1", name="ck_persistent_rooms_version"),
        Index("ix_persistent_rooms_owner_archived", "owner_user_id", "archived_at"),
    )
    ...
    code: Mapped[str] = mapped_column(
        String(6),
        ForeignKey("room_code_reservations.code", ondelete="RESTRICT"),
        nullable=False,
        unique=True,
    )
    ...
    # Stable IDs are retained even if a referenced player-owned list is later
    # deleted. Materialization then reports a visible validation error instead
    # of silently substituting bundled prompts.
