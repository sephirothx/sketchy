...
async def friends_online(ctx: HandlerContext, sid, data):
    ...
    joining the lobby channel (#873). Polled, not pushed: a client showing
    friends asks again every few seconds (R-PRESENCE-06). A guest, or an account with no friends,
    ...
    try:
        ...
        logger.exception("Could not read the friends of %s", account)
        return BUSY_ACKNOWLEDGEMENT
