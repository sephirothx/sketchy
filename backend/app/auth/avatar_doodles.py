...
drawing itself is a symbol in `frontend/public/avatars/doodles.svg` - drawn
as centrelines in `scripts/brand/avatar-doodles.mjs` and inked by
`scripts/brand/build-avatar-doodles.mjs` - painted in the disc's ink so it
takes the player's name colour. The server holds the list only to refuse a
name it does not have: nothing is uploaded, nothing is stored but the name,
and nothing about it needs moderating.
